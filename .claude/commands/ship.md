...
2. Read `CLAUDE.md` and the Development Roadmap (Section 7) in `design-doc.md`
...
- Check if the changes implement (partially or fully) a task from the Development Roadmap in `design-doc.md`. If so, include the task label in the first line (e.g., `feat(P1-T1): define Pydantic data models`). If the changes are not related to a roadmap task, omit the label.
